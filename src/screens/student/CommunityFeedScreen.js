...
export default function CommunityFeedScreen({ route, navigation }) {
  ...
  // FIXED: Enhanced refresh system
  useEffect(() => {
    const unsubscribeFocus = navigation.addListener('focus', () => {
      console.log('🎯 SCREEN FOCUSED - FORCING REFRESH');
      forceRefresh(); // Use forceRefresh instead of onRefresh
      loadCommunityDetails();
    });

    // ADDED: Register for refresh callbacks
  ...
  // FIXED: Enhanced refresh trigger
  useEffect(() => {
  ...
  // FIXED: ULTIMATE POST CREATION WITH MULTIPLE REFRESH STRATEGIES
  const handleCreatePost = () => {
    navigation.navigate('CreateCommunityPost', { 
      ...
      onPostCreated: () => {
        ...
        // Strategy 1: Immediate force refresh
        setTimeout(() => {
          console.log('💥 REFRESH 1: Immediate nuclear refresh');
          forceRefresh();
        }, 100);
        
        // Strategy 2: Secondary refresh
        setTimeout(() => {
          console.log('💥 REFRESH 2: Secondary nuclear refresh');
          forceRefresh();
        }, 500);
        
        // Strategy 3: Final refresh after real-time should have caught it
        setTimeout(() => {
          console.log('💥 REFRESH 3: Final nuclear refresh');
          forceRefresh();
        }, 1500);
        
        // Strategy 4: One more for good measure
        setTimeout(() => {
          console.log('💥 REFRESH 4: Extra nuclear refresh');
          forceRefresh();
        }, 2500);
      }
    });
